...
#include "caffe/video/pooling3d_layer.hpp"
...
void Pooling3DLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
	...
	if (pool_param.global_pooling()) {
		...
		CHECK(!pool_param.has_kernel_size() !=
			...
			<< "For filters both kernel_size and kernel_l are required.";
	}
...
void Pooling3DLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
  ...
  if (pad_h_ || pad_w_ || pad_l_) {
    ...
    CHECK_LT((pooled_length_ - 1) * stride_l_, height_ + pad_l_);
	CHECK_LT((pooled_height_ - 1) * stride_h_, height_ + pad_h_);
	CHECK_LT((pooled_width_ - 1) * stride_w_, width_ + pad_w_);
  }
  top[0]->Reshape(video_shape(bottom[0]->shape(0), channels_, pooled_length_, pooled_height_,
      pooled_width_));
  if (top.size() > 1) {
  ...
  case Pooling3DParameter_PoolMethod_MAX:
    ...
    for (int n = 0; n < bottom[0]->shape(0); ++n) {
      for (int c = 0; c < channels_; ++c) {
        ...
        if (use_top_mask) {
          top_mask += top[0]->offset(video_shape(0, 1));
        } else {
          mask += top[0]->offset(video_shape(0, 1));
        }
      }
    }
    break;
