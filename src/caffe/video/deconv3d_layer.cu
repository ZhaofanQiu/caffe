	...
	void Deconvolution3DLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
		...
		for (int i = 0; i < bottom.size(); ++i) {
			...
			for (int n = 0; n < this->num_; ++n) {
				this->backward_gpu_gemm(bottom_data + bottom[i]->offset(video_shape(n)), weight,
			...
			if (this->param_propagate_down_[0] || propagate_down[i]) {
				for (int n = 0; n < this->num_; ++n) {
					...
					if (this->param_propagate_down_[0]) {
						this->weight_gpu_gemm(top_diff + top[i]->offset(video_shape(n)),
							bottom_data + bottom[i]->offset(video_shape(n)), weight_diff);
					}
					// gradient w.r.t. bottom data, if necessary.
					if (propagate_down[i]) {
						this->forward_gpu_gemm(top_diff + top[i]->offset(video_shape(n)), weight,
