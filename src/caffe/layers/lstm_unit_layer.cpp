...
namespace caffe {
	...
	void LSTMUnitLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
		...
		CHECK_EQ(0, top.size() % 2);
		this->input_num_ = bottom.size() - 1;
		...
		for (int i = 1; i < bottom.size(); ++i) {
			...
			CHECK_EQ(bottom[0]->shape(1), bottom[i]->shape(1) * 4);
		}

		X_acts_.ReshapeLike(*bottom[0]);
		for (int i = 0; i < this->input_num_; i++)
		{
			top[i * 2]->ReshapeLike(*bottom[1]);
			top[i * 2 + 1]->ReshapeLike(*bottom[1]);
		}
	}
